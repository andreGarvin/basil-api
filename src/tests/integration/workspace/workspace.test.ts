...
test("/api/workspace/create", async t => {
  ...
  t.deepEqual(response.body.meta, { is_creator: true });
...
test("/api/workspace/create (sending invalid data)", async t => {
  ...
  t.is(workspace, null, "workspace should not exist");
});
...
test("/api/workspace/update/:workspace_id", async t => {
  const mockWorkspace = db.createMockWorkspaceInfo();
  ...
  const response = await request(app)
    .patch(`/api/workspace/update/${workspace.id}`)
    .send({
...
test("/api/workspace/update/:workspace_id (sending invalid data)", async t => {
  const response = await request(app)
    .patch(`/api/workspace/update/dfdfsds`)
    .send({
...
test("/api/workspace/update/:workspace_id (updating a workspace that does not exist)", async t => {
  const response = await request(app)
    .patch("/api/workspace/update/dfdfsds")
    .send({
...
test("/api/workspace/update/:workspace_id (updating a workspace that is archived)", async t => {
  const mockWorkspace = db.createMockWorkspaceInfo();
...
test("/api/workspace/update/:workspace_id (updating a workspace but another workspace already exists)", async t => {
  const otherMockWorkspace = db.createMockWorkspaceInfo();
  await db.createWorkspace(
    t.context.user.id,
    t.context.user.school_id,
    Object.assign(otherMockWorkspace, { scope: WorkspaceScopes.global })
  );
