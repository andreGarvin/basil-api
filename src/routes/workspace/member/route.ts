...
router.get("/invited/members/:workspace_id", (req, res, next) => {
  const body = {
    page: req.query.page,
...
router.get("/members/:workspace_id", (req, res, next) => {
  const body = {
    page: req.query.page,
    limit: req.query.limit
  };
  ...
  return workspaceMember
    .getMembers(req.state.user, req.params.workspace_id, body.page, body.limit)
    .then(response => res.status(200).json(response))
    .catch(next);
});

router.get("/search/:workspace_id", (req, res, next) => {
  const body = {
    page: req.query.page,
    limit: req.query.limit,
...
router.get("/info/:workspace_id/:member_user_id", (req, res, next) => {
  return workspaceMember
    .getMemberInfo(
      req.state.user,
      req.params.workspace_id,
...
router.put("/admin/:workspace_id/:member_user_id", (req, res, next) => {
  return workspaceMember
...
router.patch("/info/:workspace_id", (req, res, next) => {
  ...
  return workspaceMember
    ...
    .then(memberStatus => res.status(200).json({ member_status: memberStatus }))
    .catch(next);
});

router.delete("/remove/:workspace_id/:member_user_id", (req, res, next) => {
  return workspaceMember
    .removeMember(
      req.state.user,
      req.params.workspace_id,
